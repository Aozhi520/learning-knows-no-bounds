...
不过现在Cudnn一开始就由pytorch自带了，用pytorch时不用额外的下载，只需要有gpu就能用pytorch来驱动cuda，不需要直接使用cuda api。**但是如果想要研究pytorch核心代码或者编写pytorch扩展，那么就需要知道如何直接使用cuda**，**因为底层仍然是c和c++，如果想要进一步加快处理速度以及提升性能，那么cuda就要去研究**
...
张量的阶告诉我们需要多少个索引才能访问（引用）张量数据结构中包含的特定数据元素。



### **张量的轴**

张量的轴是张量的一个特定维度，张量有n个维度，就有n个轴，**即轴的数量就是维度的数量**
...
![image-20260329214742179](笔记.assets/image-20260329214742179.png)
...
参考下面的图片，**动图第一列是输入，有三个通道，第二列中三个 3x3 卷积核共同构成一个滤波器，第三列同理。滤波器数量始终等于下一层的特征图数量(输出通道数)，而每个滤波器中的卷积核数量始终等于当前层的特征图数量。**
...
权重矩阵由配音pytorch的**PyTorch LinearLayer类**自动创建， PyTorch LinearLayer类的**构造函数**用实参4和3
...
stacking张量和concatenating张量之间的差异可以用一句话描述：
...
想要理解stack之后的形状，t1.unsqueeze(dim=0)以后得形状，张量形状变成了1x3，而第1个维度的元素是数组，所以在拼接时拼接第0维，实际上是把3个数组拼接起来(首尾相连)，print的结果可能有些误导，下面的图更形象
...
class RunBuilder():
    ...
    def get_runs(params):
        ...
        for v in product(*params.values()):
            ...
            runs.append(Run(*v))  # *v表示传入的是值而不是列表本身，即解包
